...
def merge_interpretations(radar_key: str, step_size: float = 30., cold_model_strength: float = 100.) -> gpd.GeoDataFrame:
    ...
    for interp_path in interp_paths:
        data = pd.concat([data, read_interpretation(interp_path, step_size=step_size)], ignore_index=True)
...
def merge_all_interpretations(step_size: float = 30., outlier_threshold: float = 200., cold_model_strength: float = 100.) -> gpd.GeoDataFrame:
    ...
    for radar_key in tqdm.tqdm(radar_keys):
        out = merge_interpretations(radar_key=radar_key, step_size=step_size, cold_model_strength=cold_model_strength)
...
def grid_interpretations(name: str, outline: shapely.geometry.Polygon, cols: list[str] = ["thickness", "temperate_frac"]) -> dict[str, np.ndarray | dict[str, float]]:
    ...
    for col in cols:
        ...
        raster, bounds = rasters.interpolate_raster(cache_path, zcol=col, points=subset, extrapolation_distance=extrapolation_distance, res=25, outline=outline, vmin=vmin, vmax=vmax)
