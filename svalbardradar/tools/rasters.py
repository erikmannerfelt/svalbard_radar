...
def bounds_dict(dataset: rasterio.DatasetBase) -> dict[str, float]:
...
def interpolate_raster(
    ...
    if out_filepath.is_file() and _rec >= 0:
        with rasterio.open(out_filepath) as raster:
    ...
    points = points.select_dtypes(np.number).groupby("bin").mean() # pyright: ignore[reportAssignmentType]
