...
from svalbardradar.tools import statistics, paths
...
def plot_dronbreen_examples(show: bool = True):
    ...
    for i in range(axes.shape[1]):
        ...
        extent = extent=(gridded["bounds"]["left"], gridded["bounds"]["right"], gridded["bounds"]["bottom"], gridded["bounds"]["top"])
        if i == 0:
            img = axis.imshow(gridded["thickness"], cmap="Blues", zorder=2, extent=extent, vmin=0)
        else:
            img = axis.imshow(gridded["temperate_frac"] * 100, vmin=0, vmax=100, extent=extent, cmap="Reds", zorder=2)
...
def plot_model_comparison(show: bool = True):
    ...
    for i, model in enumerate(models):
        ...
        hist2 = np.histogram2d(data[f"{model}_thickness"],data["thickness"], bins=thickness_bins)[0][::-1, :]
...
def plot_glathida_comparison(show: bool = True):
    ...
    for i, group in data.groupby("glathida_group"):
        ...
        plt.scatter(group["thickness"], group["glathida_thickness"], label=label, marker=markers[i], s=9)
