...
def main():
    ...
    for survey_id, survey_pts in pts.groupby("survey_id"):
        ...
        diffs = (
            ((survey_pts.geometry.x.diff() ** 2 + survey_pts.geometry.y.diff() ** 2) ** 0.5).fillna(0).to_frame("diff")
        )
    ...
    total_modern_ground_length = surveys.loc[
        (surveys["from_date"].dt.year >= 1900) & (surveys["platform"] != "air"), "length_km"
    ].sum()
