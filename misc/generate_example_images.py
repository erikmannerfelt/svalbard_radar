...
def load_tile(radar_key, yoff, xoff):
    ...
    tile_name = Path(
        f"static/radargrams/{glacier}/{date_str}/{filename}/tiles/abslog/tile_{str(xoff).zfill(5)}_{str(yoff).zfill(5)}.jpg"
    )
...
def main():
    ...
    for image_name, props in images.items():
        ...
        img = img[
            props["ymin"] : props["ymin"] + int(height * props["scale"]),
            props["xmin"] : props["xmin"] + int(width * props["scale"]),
        ]
